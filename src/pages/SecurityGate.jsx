...
import { CheckCircle, XCircle, Clock, Car, User, Home } from 'lucide-react'
...
export default function SecurityGate() {
  ...
  const updateStatus = (id, status) => {
    setVisitors((prev) =>
      prev.map((v) =>
        v.id === id ? { ...v, status, approved_at: new Date().toISOString() } : v
      )
    )
  }
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                      PURPOSE_COLORS[v.purpose] || PURPOSE_COLORS.Other
                    }`}
                  >
                    {v.purpose}
                  </span>
